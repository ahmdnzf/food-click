            ...
            <div class="row">
                                        ...
                                        <tr>
                                            ...
                                            <td class="fp__pro_name">
                                                ...
                                                <span>{{ @$product->options->product_size['name'] }}
                                                    {{ @$product->options->product_size['price'] ? '(' . currencyPosition(number_format(@$product->options->product_size['price'],2)) . ')' : '' }}
                                                </span>
                                                @foreach ($product->options->product_options as $option)
                                                    <p>{{ $option['name'] }} ({{ currencyPosition(number_format($option['price'],2)) }})</p>
                                                @endforeach

                                            </td>

                                            <td class="fp__pro_status">
                                                <h6>{{ currencyPosition(number_format($product->price,2)) }}</h6>
                                            </td>
                                            ...
                                            <td class="fp__pro_tk">
                                                <h6 class="product_cart_total">
                                                    {{ currencyPosition(number_format(productTotal($product->rowId),2)) }}</h6>
                                            </td>
                ...
                <div class="col-lg-4 wow fadeInUp" data-wow-duration="1s">
                    <div class="fp__cart_list_footer_button">
                        <h6>total cart</h6>
                        <p>subtotal: <span id="subtotal"> {{ currencyPosition(number_format(cartTotal(),2)) }}</span></p>
                        <p>delivery: <span> RM0.00</span></p>
                        ...
                        <p class="total"><span>total:</span> <span id="final_total">
                                @if (isset(session()->get('coupon')['discount']))
                                {{ config('settings.site_currency_icon') }} {{ number_format(cartTotal() - session()->get('coupon')['discount'],2) }}
                                @else
                                {{ config('settings.site_currency_icon') }} {{ number_format(cartTotal(),2) }}
                                @endif
                            </span></p>
                        <form id="coupon_form">
                    ...
                    if (response.status === 'success') {
                        ...
                        inputField.closest("tr")
                            ...
                            .text("{{ currencyPosition(':productTotal') }}"
                                .replace(":productTotal", productTotal.toFixed(2)));
                        ...
                        $("#final_total").text("{{ config('settings.site_currency_icon') }}" +
                            response.grand_cart_total.toFixed(2));
            ...
            function removeCartProduct(rowId) {
                $.ajax({
                    ...
                    success: function(response) {
                        updateSidebarCart();
                        cartTotal = response.cart_total;
                        $('#subtotal').text("{{ config('settings.site_currency_icon') }}" + cartTotal.toFixed(2));
                        $("#final_total").text("{{ config('settings.site_currency_icon') }}" +
                            response.grand_cart_total.toFixed(2));
                    },
                    error: function(xhr, status, error) {
            ...
            function couponApply(code, subtotal) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#final_total').text("{{ config('settings.site_currency_icon') }}" + response.finalTotal.toFixed(2));
                        $couponCartHtml = `<div class="card mt-2">
                                    <div class="m-3">
                                        <span><b class="v_coupon_code">Applied Coupon:
                                                ${response.coupon_code}</b></span>
                                        <span>
                                            <button id="destroy_coupon"><i class="far fa-times"></i></button>
                                        </span>
                                    </div>
